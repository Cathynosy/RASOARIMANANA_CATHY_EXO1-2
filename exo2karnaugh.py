...
#Création de la grille de Kranaugh
...
#Regroupement des minterms adjacents groups=[]
for i in range(rows):
    for j in range(cols):
...
def get_truth_table(num_variables):
    ...
    try:
        ...
        if len(truth_table)!=2**num_variables:
            raise ValueError(f"La longueur de la table de vérité doit etre {2**num_variables}.")
        return truth_table
